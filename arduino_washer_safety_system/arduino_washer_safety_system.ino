 ...
 *  - 대기 상태에서 절전모드 진입 가능, 문을 여닫으면 깨어남
 *
 * 핀맵
 *  D2      리드스위치 DO (도어, 외부 인터럽트 - 절전 해제용)
...
// ---------------- 상태 머신 ----------------
enum State {
  IDLE, FILLING, WASHING, SPINNING, DONE, ALARM, PAUSED
};

State currentState = IDLE;

// ---------------- 시간 설정 ----------------
const unsigned long LCD_INTERVAL  = 300;    // LCD 갱신 주기
const unsigned long FILL_DURATION = 20000;  // 급수 제한시간. 손으로 물 붓는 걸 감안해 넉넉히
const unsigned long WASH_DURATION = 10000;  // 세탁 시간
const unsigned long SPIN_DURATION = 8000;   // 탈수 시간

// ---------------- 물높이 (실측 기반) ----------------
// 이 센서(전도성 접점식)는 물이 조금만 닿아도 값이 400대까지 튀는
// 비선형 특성이 있어서, 단순 비례식 대신 실측 4개 지점을
// 구간별 직선 보간해서 %를 계산한다.
// 실측값: 건조 0 / 소량 410 / 중간 520 / 목표 수위 660
const int waterAdcPoints[]     = {0, 410, 520, 660};
const int waterPercentPoints[] = {0, 33, 67, 100};
const int WATER_POINTS_COUNT   = 4;
const int WATER_FULL_THRESHOLD = 660;   // 이 값 도달 시 급수 정지
...
// 도어 인터럽트: 절전모드에서 깨어나기 위한 용도라 핸들러는 비어 있다.
// 도어 감시 자체는 2초 디바운스가 필요해서 loop에서 폴링으로 처리한다.
void doorISR() {}
...
// 절전모드 (AVR power-down). 문을 여닫으면 D2 인터럽트로 깨어난다.
// delay는 안내 문구를 보여주는 용도로만 썼다.
// 어차피 이 직후 MCU 전체가 멈추는 구간이라 여기서의 blocking은 의미가 없다.
void enterSleep() {
  stopMelodyPlay();
  lcd.clear();
  lcd.print("Sleep Mode...");
  lcd.setCursor(0, 1);
  lcd.print("Open door to wake");
  delay(500);
  ...
  sleep_enable();
  sleep_mode();      // 여기서 정지, 인터럽트 대기
  sleep_disable();   // 깨어나면 여기부터 재개
...
void enterPaused(int reason) {
  ...
  currentState = PAUSED;
  saveState(PAUSED);
  Serial.println(reason == 1 ? "PAUSED (door opened)" :
                  reason == 2 ? "PAUSED (imbalance)" : "PAUSED (button)");
...
void setup() {
  ...
  // 마지막 상태 복원. 동작 중이거나 완료/알람/일시정지로 저장돼 있으면
  // 전부 대기로 되돌린다. 이 처리가 없으면 재부팅해도 그 화면에 갇힌다.
  // (실제로 DONE 상태로 저장된 채 껐다 켰더니 계속 완료 화면만 떠서 추가한 로직)
  byte saved = EEPROM.read(EEPROM_ADDR_STATE);
  if (saved <= PAUSED) {
    currentState = (State)saved;
    if (isRunningState(currentState) || currentState == DONE ||
        currentState == ALARM || currentState == PAUSED) {
      currentState = IDLE;
    }
  } else {
    currentState = IDLE;   // EEPROM 초기값(255) 등 이상한 값 방어
  }
