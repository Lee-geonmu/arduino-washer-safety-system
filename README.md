...
| 영상 | 내용 |
|---|---|
| [영상 1 - 링크 추가 예정] | 도어 인터록(문 열린 상태에서 시작 불가 → "Close the door") 확인 후 정상 사이클 완주 |
| [영상 2 - 링크 추가 예정] | 짧은 문 접촉은 무시(디바운스) → 2초 이상 열림 시 안전 정지 → 문을 닫아야만 재개 가능 → 버튼 일시정지/재개 |
| [영상 3 - 링크 추가 예정] | 탈수 중 진동 감지 → 즉시 정지 + 경보 → 재개 시 탈수를 처음부터 재시도 |
